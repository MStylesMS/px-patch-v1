...
static const static_asset_t ASSET_MONITOR = {
...
static char s_prop_id[32] = "Patch32Prop";
...
static esp_err_t save_connection_cfg_nvs(void)
{
    ...
    patch_engine_get_config_json(prop_cfg, sizeof(prop_cfg));
    if (!conn_cfg_lock()) {
        return ESP_ERR_TIMEOUT;
    }
    cfg_copy = s_conn_cfg;
...
static void cpu_temp_init(void)
{
    /* Classic ESP32 has no driver/temperature_sensor.h peripheral. */
...
unauthorized:
    httpd_resp_set_hdr(req, "WWW-Authenticate", "Basic realm=\"PX-Patch-V1\"");
...
static void build_default_identity(void)
{
    ...
    if (err == ESP_OK) {
        snprintf(s_prop_id, sizeof(s_prop_id), "Patch32Prop");
    }
...
static const char *patch_led_hint_name_local(patch_led_hint_t hint);
...
static void mqtt_publish_state(bool retained)
{
    ...
    state_json = (char *)malloc(PATCH_STATE_JSON_MAX);
    if (!state_json) {
        ESP_LOGW(TAG, "mqtt_publish_state: out of memory");
        return;
    }

    mqtt_build_topic(topic, sizeof(topic), "state");
    patch_engine_get_state_json(state_json, PATCH_STATE_JSON_MAX);
...
static bool mqtt_publish_engine_events(bool publish_state_after)
{
    ...
    while (patch_engine_pop_event_json(event_json, sizeof(event_json))) {
...
static void mqtt_publish_legacy_heartbeat(void)
{
    char topic[sizeof(s_conn_cfg.mqtt_prop_announce_topic)];
    char payload[220];
    ...
    copy_bounded_local(topic, sizeof(topic), s_conn_cfg.mqtt_prop_announce_topic);
    conn_cfg_unlock();
    ...
    /* control.js watches /Paradox/Props for {"id":"Patch32Prop"}. */
    snprintf(payload,
             sizeof(payload),
             "{\"id\":\"%s\",\"ip\":\"%s\",\"ts\":%lld,\"status\":\"online\",\"version\":\"%s\"}",
             s_prop_id,
             ip_text,
             (long long)(esp_timer_get_time() / 1000),
             app ? app->version : "0.02");
    mqtt_publish_or_warn(topic, payload, 0, false);
}
...
static void mqtt_publish_announce(void)
{
    ...
    patch_battery_snapshot_t battery_snap;
    esp_netif_t *sta_netif = esp_netif_get_handle_from_ifkey("WIFI_STA_DEF");
    const esp_app_desc_t *app = esp_app_get_description();
    ...
    patch_engine_get_battery_snapshot(&battery_snap);
    ...
    snprintf(announce,
             ...
             patch_led_hint_name_local(patch_engine_get_led_hint()),
             app->version,
             app->version,
             app->date,
             app->time,
...
static void mqtt_apply_follower_payload(const char *payload, int payload_len)
{
    ...
    patch_engine_get_state_json(local_state_json, sizeof(local_state_json));
    ...
    if (local_remaining_s >= 0 &&
        ...
        if (patch_engine_handle_command_json(cmd, response, sizeof(response)) == ESP_OK) {
    ...
    if (has_pause && game_paused) {
        if (strcmp(local_state, "countdown") == 0) {
            if (patch_engine_handle_command_json("{\"command\":\"pause\"}", response, sizeof(response)) == ESP_OK) {
        ...
        if (patch_engine_handle_command_json("{\"command\":\"resume\"}", response, sizeof(response)) == ESP_OK) {
            mqtt_publish_follow_event("commandOverridden",
                                      local_remaining_s,
                                      remote_remaining_s,
...
static void mqtt_handle_command_payload(const char *payload, int payload_len)
{
    ...
    if (patch_engine_handle_command_json(cmd_json, response, sizeof(response)) == ESP_OK) {
...
static void mqtt_state_task(void *arg)
{
    ...
    while (true) {
        ...
        (void)mqtt_publish_engine_events(false);

        /* Drain and publish warning messages (e.g., low battery warnings) */
        while (patch_engine_pop_warning_message(warning_msg, sizeof(warning_msg))) {
...
static esp_err_t state_get_handler(httpd_req_t *req)
{
    ...
    payload = (char *)malloc(PATCH_STATE_JSON_MAX);
    if (!payload) {
        httpd_resp_send_err(req, HTTPD_500_INTERNAL_SERVER_ERROR, "Out of memory");
        return ESP_ERR_NO_MEM;
    }

    patch_engine_get_state_json(payload, PATCH_STATE_JSON_MAX);
    httpd_resp_set_type(req, "application/json");
...
static void build_unified_config_json(char *out, size_t out_size)
{
    ...
    patch_engine_get_config_json(prop_payload, sizeof(prop_payload));
...
static esp_err_t config_defaults_get_handler(httpd_req_t *req)
{
    ...
    patch_engine_get_default_config_json(payload, sizeof(payload));
    httpd_resp_set_type(req, "application/json");
...
static esp_err_t command_post_handler(httpd_req_t *req)
{
    ...
    engine_err = patch_engine_handle_command_json(body, response, sizeof(response));
...
static esp_err_t config_post_handler(httpd_req_t *req)
{
    ...
    engine_err = patch_engine_apply_config_json(body, persist, response, sizeof(response));
    if (engine_err == ESP_ERR_TIMEOUT) {
        cJSON_Delete(root);
        free(body);
        return send_json_error(req, "503 Service Unavailable", "busy");
...
static esp_err_t config_restore_post_handler(httpd_req_t *req)
{
    ...
    engine_err = patch_engine_restore_defaults(false, response, sizeof(response));
    if (engine_err == ESP_ERR_TIMEOUT) {
        return send_json_error(req, "503 Service Unavailable", "busy");
    }
    if (engine_err != ESP_OK) {
...
static esp_err_t connection_get_handler(httpd_req_t *req)
{
    ...
    char ap_ssid[33] = "Paradox-PXPatchV1";
    connection_cfg_t cfg_snap;
...
static esp_err_t device_details_get_handler(httpd_req_t *req)
{
    ...
    patch_engine_get_state_json(state_json, sizeof(state_json));
...
static esp_err_t ws_handler(httpd_req_t *req)
{
    ...
    if (payload[0] == '\0') {
        ...
        esp_err_t engine_err = patch_engine_handle_command_json(payload, reply_buf, sizeof(reply_buf));
        if (engine_err == ESP_ERR_TIMEOUT) {
...
static void wifi_on_sta_connected(const char *ip_text, void *ctx)
{
    ...
    /* Notify fuse engine of WiFi connection for state snapshot RSSI/SSID. */
    svc_wifi_get_status(&wifi_status);
    if (wifi_status.connected) {
        patch_engine_notify_wifi_connected(wifi_status.rssi);
        patch_engine_set_wifi_ssid(wifi_status.ssid);
    }
...
static void wifi_on_sta_disconnected(int reason, const char *reason_text, void *ctx)
{
    ...
    /* Notify fuse engine of WiFi disconnection. */
    patch_engine_notify_wifi_disconnected();
...
esp_err_t web_ui_start(void)
{
...

    {
        ...
        snprintf(ap_ssid, sizeof(ap_ssid), "Paradox-PXPatchV1-%02X%02X", mac[4], mac[5]);
         ...
         * after STA gets an IP (classic ESP32 APSTA hurts TCP throughput). */
    ...
    httpd_uri_t monitor_html_uri = {
        .uri = "/monitor.html",
        .method = HTTP_GET,
        .handler = static_asset_handler,
        .user_ctx = (void *)&ASSET_MONITOR,
    };
    httpd_register_uri_handler(server, &monitor_html_uri);
    ...
    xTaskCreate(mqtt_state_task, "mqtt_state", 8192, NULL, 5, NULL);
