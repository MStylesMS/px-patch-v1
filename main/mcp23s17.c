...
esp_err_t mcp23s17_init(void)
{
    ...
    /* BANK=0, HAEN on (addr 0). */
    (void)mcp_write8(REG_IOCON, 0x08);
    s_iodir = 0xFFFF;
    s_gppu = 0xFFFF;
    s_olat = 0x0000;
    err = flush_dir_olat();
    ...
    s_ok = true;
    ESP_LOGI(TAG, "MCP23S17 addr 0 ready on HSPI");
    return ESP_OK;
}
...
void mcp23s17_pin_output_low(int port)
{
    ...
    (void)mcp_write16(REG_IODIRA, s_iodir);
    (void)mcp_write16(REG_OLATA, s_olat);
}
...
void mcp23s17_pin_input_pullup(int port)
{
    ...
    s_gppu |= mask;
    (void)mcp_write16(REG_IODIRA, s_iodir);
    (void)mcp_write16(REG_GPPUA, s_gppu);
}
